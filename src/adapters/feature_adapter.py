...
class FeatureAdapter(BaseService, IFeatureManager):
        ...
        try:
                ...
                for col in current_columns.intersection(reference_columns):
                    if col in reference_dtypes:
                        ...
                        if current_dtype != reference_dtype:
                            warnings.append(
                                f"Data type drift in column '{col}': {current_dtype} vs {reference_dtype}"
                            )
            ...
            if "feature_count" in reference_schema:
                ...
                if abs(actual_count - expected_count) > 5:  # Allow some tolerance
                    warnings.append(
                        f"Feature count drift: {actual_count} vs expected {expected_count}"
                    )

            drift_detected = (
                len(errors) > 0 or len(warnings) > 2
            )  # Significant drift if many warnings
    ...
    def detect_drift(
        ...
        try:
            ...
            self._log_info(
                f"Drift detection completed: {drift_count}/{len(common_cols)} features show drift"
            )
            return drift_results
            ...
            for col in data.columns:
                ...
                if pd.api.types.is_numeric_dtype(data[col]):
                    col_info.update(
                        {
                            "min": (
                                float(data[col].min()) if not data[col].isnull().all() else None
                            ),
                            "max": (
                                float(data[col].max()) if not data[col].isnull().all() else None
                            ),
                            "mean": (
                                float(data[col].mean()) if not data[col].isnull().all() else None
                            ),
                        }
                    )
