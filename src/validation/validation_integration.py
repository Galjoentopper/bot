...
class ValidationManager:
    ...
    def __init__(
        ...
        self.advanced_drift_monitor = AdvancedFeatureDriftMonitor(
            self.config.get("advanced_drift", {})
        )
        self.enhanced_logger = EnhancedValidationLogger(
            log_dir=str(self.config_dir / "logs"), config=self.config.get("logging", {})
        )
    ...
    def _load_config(self) -> Dict:
        ...
        default_config = {
            ...
            "model_specific_settings": {
                "gru": {"strict_validation": True, "drift_sensitivity": "medium"},
                "lightgbm": {"strict_validation": True, "drift_sensitivity": "medium"},
                "ppo": {
    ...
    def start_monitoring(self):
        ...
        if self.config.get("drift_monitoring_enabled", True):
            ...
            self.logger.logger.info(
                "Drift monitoring started for production server with trained models"
            )
        else:
    ...
    def validate_model_input(
        ...
        if self.config.get("drift_monitoring_enabled", True):
            ...
            advanced_drift_result = self.advanced_drift_monitor.add_current_data(
        ...
        self.enhanced_logger.log_schema_decision(
            ...
            metadata={
                "drift_detected": validation_result.get("advanced_drift", {}).get(
    ...
    def _handle_drift_alert(self, alert: DriftAlert):
        ...
        self.logger.logger.log(
            log_level, f"DRIFT ALERT [{alert.severity.upper()}]: {alert.message}"
        )
    ...
    def create_validation_decorator(
        self, model_type: str, symbol: str, context: str = "prediction"
    ):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                if data_arg is not None:
                    validation_result = self.validate_model_input(
                        data_arg, model_type, symbol, context
                    )

                    # Check if we should proceed based on validation
                    model_settings = self.config.get("model_specific_settings", {}).get(
                        model_type, {}
                    )
                    strict_validation = model_settings.get("strict_validation", True)
                    ...
                    if not validation_result["valid"] and strict_validation:
                        raise ValueError(
                            f"Validation failed for {model_type} model: {validation_result['errors']}"
                        )
    ...
    def get_validation_summary(self, hours: int = 24) -> Dict:
        ...
        recent_validations = [
            v for v in self.validation_history if pd.to_datetime(v["timestamp"]) >= cutoff
        ]
    ...
    def generate_validation_report(self, output_file: Optional[str] = None) -> Dict:
        ...
        report = {
            ...
            "recommendations": self._generate_recommendations(
                schema_report, drift_summary, recent_alerts
            ),
        }
    ...
    def _generate_recommendations(
        self, schema_report: Dict, drift_summary: Dict, recent_alerts: List
    ) -> List[str]:
