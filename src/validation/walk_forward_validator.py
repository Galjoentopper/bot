...
class WalkForwardValidator:
    ...
    def generate_splits(
        self, data_length: int, timestamps: Optional[pd.DatetimeIndex] = None
    ...
    def _generate_expanding_splits(
        self, data_length: int, timestamps: Optional[pd.DatetimeIndex]
    ) -> List[ValidationSplit]:
    ...
    def validate_model(
        ...
        for i, split in enumerate(splits):
            ...
            try:
                ...
                logger.info(
                    f"Fold {i+1} completed - Val RMSE: {val_metrics.get('rmse', 'N/A'):.6f}"
                )
        ...
        results = WalkForwardResults(
            validation_results=validation_results, total_validation_time=total_time
        )
    ...
    def _analyze_performance_stability(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        if rmse_values:
            stability_metrics["rmse_stability"] = {
                "coefficient_of_variation": (
                    np.std(rmse_values) / np.mean(rmse_values)
                    if np.mean(rmse_values) > 0
                    else float("inf")
                ),
        ...
        if sharpe_values:
            stability_metrics["sharpe_stability"] = {
                "coefficient_of_variation": (
                    np.std(sharpe_values) / abs(np.mean(sharpe_values))
        ...
        if dir_acc_values:
            stability_metrics["directional_accuracy_stability"] = {
                "coefficient_of_variation": (
                    np.std(dir_acc_values) / np.mean(dir_acc_values)
                    if np.mean(dir_acc_values) > 0
                    else float("inf")
                ),
        ...
        cv_scores = [
            s.get("coefficient_of_variation", float("inf")) for s in stability_metrics.values()
        ]
        stability_metrics["overall_stability_score"] = np.mean(
            [cv for cv in cv_scores if cv != float("inf")]
        )

        return stability_metrics

    def _analyze_regime_performance(
        self, results: List[ValidationResult], timestamps: pd.DatetimeIndex
    ) -> Dict[str, Any]:
    ...
    def plot_performance_over_time(
        self, results: WalkForwardResults, save_path: Optional[str] = None
    ):
