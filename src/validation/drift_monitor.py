...
class FeatureDriftMonitor:
    ...
    def _aggregate_statistics(self, stats_list: List[Dict]) -> Dict:
        ...
        agg_stats = {
            "timestamp_range": (
                stats_list[0]["timestamp"],
                stats_list[-1]["timestamp"],
            ),
        ...
        for feature in all_features:
            means = [
                stats["mean"].get(feature, np.nan)
                for stats in stats_list
                if feature in stats.get("mean", {})
            ]
            stds = [
                stats["std"].get(feature, np.nan)
                for stats in stats_list
                if feature in stats.get("std", {})
            ]
            mins = [
                stats["min"].get(feature, np.nan)
                for stats in stats_list
                if feature in stats.get("min", {})
            ]
            maxs = [
                stats["max"].get(feature, np.nan)
                for stats in stats_list
                if feature in stats.get("max", {})
            ]
                ...
                if baseline_std > 0:
                    ...
                    if z_score > thresholds["mean_shift"]:
                        ...
                        result["details"].append(
                            f"Mean shift in {feature}: z-score = {z_score:.2f}"
                        )
            ...
            if feature in current.get("std", {}):
                ...
                if baseline_var > 0:
                    ...
                    if (
                        ...
                        or var_ratio < 1 / thresholds["variance_ratio"]
                    ):
                        result["drift_detected"] = True
                        result["details"].append(
                            f"Variance change in {feature}: ratio = {var_ratio:.2f}"
                        )
        ...
        for feature in baseline.get("mean", {}):
            if feature in current.get("mean", {}):
                ...
                if divergence > thresholds["js_divergence"]:
                    result["drift_detected"] = True
                    result["details"].append(
                        f"Distribution change in {feature}: divergence = {divergence:.3f}"
                    )
    ...
    def _create_alert(self, model_type: str, symbol: str, drift_type: str, drift_result: Dict):
        ...
        self.alert_timestamps[alert_key] = [
            ts for ts in self.alert_timestamps[alert_key] if current_time - ts < 60
        ]
        ...
        if alert.severity in ["critical", "high"]:
            self.logger.logger.warning(f"DRIFT ALERT: {alert.message} (Severity: {alert.severity})")
        ...
        try:
            ...
            for key, baseline in self.baseline_stats.items():
                ...
                if "timestamp_range" in serializable_baseline:
                    ...
                    serializable_baseline["timestamp_range"] = [
                        start.isoformat(),
                        end.isoformat(),
                    ]
