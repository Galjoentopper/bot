...
class SchemaValidator:
    ...
    def _load_model_specific_schemas(self) -> Dict[str, Dict]:
        ...
        for model_type, schema in global_schemas.items():
            if model_type not in schemas or schemas[model_type] == self.default_model_schemas.get(
                model_type
            ):
                schemas[model_type] = schema
    ...
    def _create_default_expectations(self):
        ...
        for model_type, schema in self.model_schemas.items():
            ...
            try:
                ...
                suite.add_expectation(
                    gx.expectations.ExpectTableColumnCountToEqual(value=expected_features)
                )
                ...
                for col_type in schema["feature_types"]:
                    suite.add_expectation(
                        gx.expectations.ExpectColumnValuesToBeOfType(
                            column="*", type_=col_type  # Apply to all columns
                        )
                    )

                # No null values expectation
                suite.add_expectation(gx.expectations.ExpectColumnValuesToNotBeNull(column="*"))
                ...
                self.logger.logger.error(
                    f"Failed to create expectation suite for {model_type}: {e}"
                )

    def validate_model_input(
        self, data: pd.DataFrame, model_type: str, symbol: str
    ) -> Dict[str, Any]:
        ...
        if (
            self.has_gx
            and gx is not None
            and self.gx_context is not None
            and RuntimeBatchRequest is not None
        ):
    ...
    def _validate_with_gx(self, data: pd.DataFrame, model_type: str, result: Dict):
        ...
        try:
            ...
            result["metrics"]["gx_success_rate"] = validation_results.statistics.get(
                "success_percent", 0
            )
    ...
    def _validate_statistical_properties(
        self, data: pd.DataFrame, model_type: str, symbol: str, result: Dict
    ):
    ...
    def create_validation_report(self, validation_results: List[Dict]) -> Dict:
        ...
        report = {
            ...
            "drift_detections": sum(
                1 for r in validation_results if r.get("metrics", {}).get("drift_detected", False)
            ),
            "summary": {},
