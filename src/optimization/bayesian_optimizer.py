...
from .financial_hyperopt import (
    AssetClass,
    FinancialHyperparameterOptimizer,
    FinancialMLConstraints,
    MarketRegime,
)
...
class FinancialBayesianOptimizer:
    ...
    def _apply_financial_constraints(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "learning_rate" in params:
            params["learning_rate"] = np.clip(
                params["learning_rate"], 1e-6, constraints.max_learning_rate
            )

        # Regularization constraints
        if "dropout" in params:
            params["dropout"] = np.clip(params["dropout"], constraints.min_regularization, 0.8)
    ...
    def _save_progress(self, optimization_history: List[Dict], filepath: str):
        ...
        progress_data = {
            "optimization_history": optimization_history,
            "best_result": (
                min(optimization_history, key=lambda x: x["score"])
    ...
    def get_convergence_analysis(self, result: BayesianOptimizationResult) -> Dict[str, Any]:
        ...
        improvement = (
            (initial_score - final_score) / abs(initial_score) if initial_score != 0 else 0
        )
