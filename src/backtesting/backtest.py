...
class Backtester:
    ...
    def walk_forward_analysis(
        ...
        while current_start + train_size + test_size <= total_periods:
            ...
            try:
                ...
                logger.info(
                    f"Walk-forward period {len(results)}: "
        ...
        return {"periods": results, "aggregate_metrics": aggregate_metrics}
    ...
    def _calculate_metrics(
        self, portfolio_values: pd.Series, returns: pd.Series, drawdowns: pd.Series
    ) -> Dict[str, float]:
        """
    ...
    def generate_report(self, results: BacktestResults) -> str:
        ...
        report.append(
            f"Final Portfolio Value: ${results.metrics.get('final_portfolio_value', 0):,.2f}"
        )
        ...
        report.append(
            f"Annualized Volatility: {results.metrics.get('annualized_volatility', 0):.2%}"
        )
