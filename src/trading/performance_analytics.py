...
class PerformanceAnalyzer:
    ...
    def record_trade(self, trade_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Trade recorded: {trade_data['symbol']} {trade_data['action']} "
    ...
    def calculate_comprehensive_metrics(
        ...
        try:
            ...
            position_value = sum(
                pos * current_prices.get(symbol, 0.0) for symbol, pos in current_positions.items()
            )
                            ...
                            completed_trades.append(
                                {
                                    ...
                                    "pnl": pnl,
                                    "pnl_pct": (
                                        pnl / (buy.get("price", 1.0) * trade_qty)
                                        if buy.get("price", 0.0) > 0
                                        else 0.0
                                    ),
                                    "quantity": trade_qty,
            ...
            if len(self.portfolio_snapshots) > 0:
                ...
                if positions:
                    max_position_pct = max(
                        pos.get("pct_portfolio", 0) for pos in positions.values()
                    )
    ...
    def _generate_improvement_suggestions(self, metrics: PerformanceMetrics) -> List[str]:
        ...
        try:
            ...
            if metrics.annualized_return < 0.05:
                suggestions.append(
                    "Consider increasing position sizes or improving signal quality to boost returns"
                )
            ...
            if metrics.sharpe_ratio < 1.0:
                suggestions.append(
                    "Focus on risk-adjusted returns by improving signal-to-noise ratio"
                )
            ...
            if len(self.portfolio_snapshots) > 0:
                ...
                if latest_snapshot.get("cash_pct", 0) > 0.3:
                    ...
                    suggestions.append(
                        "Maintain higher cash reserves for opportunities and risk management"
                    )
        ...
        try:
            ...
            for symbol, stats in symbol_stats.items():
                ...
                stats["avg_pnl_per_trade"] = (
                    stats["total_pnl"] / stats["trades"] if stats["trades"] > 0 else 0.0
                )
    ...
    def _calculate_volatility_percentile(self, current_vol: float) -> float:
        ...
        try:
            ...
            percentile = (
                sum(1 for v in historical_vols if v <= current_vol) / len(historical_vols)
            ) * 100
