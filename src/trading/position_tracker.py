            ...
            if trade.side == OrderSide.BUY:
                ...
                while remaining_sell > 0 and buy_queue:
                    ...
                    if buy_quantity <= remaining_sell:
                        ...
                        ) * sell_costs  # Proportional sell costs
...
class PositionTracker:
    ...
    def validate_buy_order(
        ...
        if position_value > portfolio_value * self.max_position_size:
            return (
                False,
                f"Position size exceeds limit: {self.max_position_size * 100}% of portfolio",
            )
    ...
    def validate_sell_order(
        ...
        if current_quantity < quantity:
            return (
                False,
                f"Insufficient shares: trying to sell {quantity}, but only own {current_quantity}",
            )
    ...
    def execute_buy(
        ...
        logger.info(
            f"Buy executed: {quantity} {symbol} @ ${price:.2f}, total cost: ${total_cost:.2f}"
        )
    ...
    def execute_sell(
        ...
        logger.info(
            f"Sell executed: {quantity} {symbol} @ ${price:.2f}, net proceeds: ${net_proceeds:.2f}"
        )
            ...
            data.append(
                {
                    ...
                    "total_cost": (
                        trade.total_cost if trade.side == OrderSide.BUY else -trade.net_proceeds
                    ),
                }
            )
