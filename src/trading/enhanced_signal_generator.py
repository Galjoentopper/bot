...
class EnhancedSignalGenerator:
    ...
    def __init__(self, config: Dict[str, Any], profit_optimizer: ProfitOptimizer):
        ...
        self.base_model_weights = signal_config.get('model_weights', {
            'lightgbm': 0.55,
            'gru': 0.35,
            'ppo': 0.1
        })
        ...
        self.volatility_threshold_multiplier = signal_config.get('volatility_threshold_multiplier', 1.5)
        ...
        logger.info(f"Enhanced signal generator initialized with confidence threshold: {self.confidence_threshold}")
        logger.info(f"Adaptive model weighting: {'enabled' if self.weight_adaptation_enabled else 'disabled'}")
    ...
    def update_model_performance(self, symbol: str, model_type: str, prediction: float, 
                               actual_return: float, trade_profitable: bool) -> None:
        ...
        try:
            ...
            if len(history['predictions']) > self.performance_window:
                ...
                history['profitable_trades'] = history['profitable_trades'][-self.performance_window:]
            ...
            if len(history['predictions']) >= 10:  # Minimum samples for reliable metrics
                ...
                correct_directions = sum(1 for p, a in zip(pred_directions, actual_directions) if p == a)
                history['accuracy_score'] = correct_directions / len(pred_directions)
                
                # Profit rate
                history['profit_rate'] = sum(history['profitable_trades']) / len(history['profitable_trades'])
            ...
            if total_score > 0:
                ...
                for model_type in self.base_model_weights.keys():
                    if model_type in composite_scores:
                        ...
                        # Gradual adaptation
                        new_weight = (1 - self.weight_adaptation_rate) * current_weight + \
        ...
        try:
            ...
            # Generate SELL signals for existing positions - More aggressive selling
                ...
                # Weak positive in bearish market - reduce exposure
                      ...
                      0 < context_adjusted_prediction < buy_threshold * 0.3):
                ...
                elif (market_context.volatility > 0.04 and 
                      abs(context_adjusted_prediction) < buy_threshold * 0.5):
