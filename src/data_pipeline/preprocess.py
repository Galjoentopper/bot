...
class DataPreprocessor:
    ...
    def __init__(self, scaler_type: str = "standard", imputer_strategy: str = "median"):
        ...
        logger.info(
            f"DataPreprocessor initialized with {scaler_type} scaler and {imputer_strategy} imputer"
        )
    ...
    def _get_scaler(self, scaler_type: str):
        ...
        if scaler_type not in scalers:
            raise ValueError(
                f"Unknown scaler type: {scaler_type}. Choose from {list(scalers.keys())}"
            )
    ...
    def fit(
        ...
        self.is_fitted = True
        logger.info(
            f"Preprocessor fitted on {X_array.shape[0]} samples with {X_array.shape[1]} features"
        )
    ...
    def transform(self, X: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        if inf_count > 0:
            logger.warning(
                f"{inf_count} infinite values found after scaling, clipping to finite range"
            )
    ...
    def create_sequences(
        ...
        if len(X) < sequence_length:
            raise ValueError(
                f"Data length ({len(X)}) is less than sequence_length ({sequence_length})"
            )
