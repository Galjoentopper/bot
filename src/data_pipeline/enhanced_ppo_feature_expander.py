...
class EnhancedPPOFeatureExpander:
    ...
    def __init__(self):
        ...
        self.timeframes = [1, 3, 6, 12, 24]  # hours for 30m candles: 2, 6, 12, 24, 48 periods
    ...
    def _add_multiframe_returns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, (timeframe, periods) in enumerate(zip(self.timeframes, self.periods)):
            ...
            df[f'log_return_{timeframe}h'] = np.log(df['close'] / df['close'].shift(periods)).shift(-periods)
    ...
    def _add_cost_adjusted_targets(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, (timeframe, periods) in enumerate(zip(self.timeframes, self.periods)):
            ...
            total_cost = (self.transaction_cost_bps + self.slippage_bps +
                         self.funding_cost_bps * periods) / 10000
            ...
            df[f'risk_reward_{timeframe}h'] = df[f'cost_adj_return_{timeframe}h'] / (volatility + 1e-8)
    ...
    def _add_risk_adjusted_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, (timeframe, periods) in enumerate(zip(self.timeframes, self.periods)):
            ...
            df[f'info_ratio_{timeframe}h'] = active_return.rolling(50).mean() / (tracking_error + 1e-8)
    ...
    def get_feature_importance_groups(self) -> Dict[str, List[str]]:
        ...
        groups = {
            ...
            'cost_adjusted': [f for f in self.feature_names if 'cost_adj' in f or 'profitable' in f],
