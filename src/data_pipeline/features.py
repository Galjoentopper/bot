...
class FeatureEngine:
    ...
    def generate_all_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ["open", "high", "low", "close"]:
            if col in features_df.columns:
                features_df[col] = np.clip(
                    features_df[col], 0.01, 100000
                )  # $0.01 to $100k hard bounds
        ...
        logger.info(
            f"Final data validation: NaN={features_df.isnull().sum().sum()}, Inf={np.isinf(features_df.select_dtypes(include=[np.number])).sum().sum()}"
        )
    ...
    def _add_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        stoch_k, stoch_d = self._calculate_stochastic(df, stoch_k_period, stoch_d_period)
        df["stoch_k"] = stoch_k
        df["stoch_d"] = stoch_d
    ...
    def _add_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["volume_roc_10"] = np.clip(
            volume_roc_10.replace([np.inf, -np.inf], [10, -0.9]), -0.99, 50
        )
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in volatility_periods:
            ...
            df[f"hist_vol_{period}"] = df[f"volatility_{period}"] * np.sqrt(
                365 * 24 * 4
            )  # 15-min intervals
    ...
    def prepare_model_input(
        self, df: pd.DataFrame, sequence_length: int = 20
        ...
        logger.info(
            f"Prepared {feature_array.shape[1]} features for {feature_array.shape[0]} samples"
        )

        return feature_array, list(feature_df.columns)

    def validate_feature_consistency(
        self, df: pd.DataFrame, expected_count: int = None
    ) -> pd.DataFrame:
        """
        ...
        if expected_count is not None:
            logger.warning(
                f"validate_feature_consistency called with hardcoded expected_count={expected_count}. "
    ...
    def pad_features_for_model(
        self, df: pd.DataFrame, model_type: str, symbol: str = None
    ) -> pd.DataFrame:
        """
    ...
    def _align_features_with_model_metadata(
        self, df: pd.DataFrame, model_type: str, symbol: str
        ...
        try:
                    ...
                    if feature_file.exists():
                        try:
                            ...
                            if expected_features:
                                logger.info(
                                    f"Found model-specific features from {feature_file}: {len(expected_features)} features"
                                )
                                break
                        except Exception as e:
            ...
            try:
                ...
                logger.info(
                    f"Successfully aligned features for {model_type}_{symbol}: {len(expected_features)} features"
                )
                return aligned_df
            except KeyError as e:
                logger.warning(
                    f"Could not reorder all expected features for {model_type}_{symbol}: {e}"
                )
                ...
                logger.info(
                    f"Partially aligned features for {model_type}_{symbol}: {len(available_features)} available, {len(missing_features)} filled with zeros"
                )
                return result_df
    ...
    def _validate_and_clean_source_data(self, df: pd.DataFrame) -> pd.DataFrame:
            ...
            if col in clean_df.columns:
                ...
                if nan_count > 0:
                    ...
                    if remaining_nan > 0:
                        ...
                        logger.warning(
                            f"Filled {remaining_nan} remaining NaN values in {col} with median {col_median:.2f}"
                        )
        ...
        for col in ["open", "high", "low", "close"]:
            if col in clean_df.columns:
                ...
                needs_cleaning = (
                    col_max > upper_bound
                    or col_min < lower_bound
                    or col_max > 500000
                    or col_min < 0
                )  # Emergency absolute bounds

                if needs_cleaning:
                    logger.error(
                        f"CRITICAL: Extreme values in {col} - min: {col_min:.2f}, max: {col_max:.2f}"
                    )
                    ...
                    if extreme_count > 0:
                        logger.warning(
                            f"Aggressively clipping {extreme_count}/{before_count} extreme values in {col} to [{lower_bound:.2f}, {upper_bound:.2f}]"
                        )
        ...
        if "volume" in clean_df.columns:
            ...
            if vol_max > vol_median * 100:
                logger.warning(
                    f"Large volume spike detected - max: {vol_max:.2f}, median: {vol_median:.2f}"
                )
        ...
        if all(col in clean_df.columns for col in ["open", "high", "low", "close"]):
            ...
            if invalid_high > 0 or invalid_low > 0:
                logger.warning(
                    f"Invalid OHLC relationships: {invalid_high} high violations, {invalid_low} low violations"
                )

                # Fix invalid relationships
                clean_df["high"] = np.maximum(
                    clean_df["high"], np.maximum(clean_df["open"], clean_df["close"])
                )
                clean_df["low"] = np.minimum(
                    clean_df["low"], np.minimum(clean_df["open"], clean_df["close"])
                )
    ...
    def _apply_robust_feature_validation(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            ...
            if col_abs_max > 500000 or (
                col_std > 0 and col_abs_max > abs(col_median) + 50 * col_std
            ):
                extreme_features.append((col, col_abs_max))
                logger.warning(
                    f"Extreme values in feature '{col}': max_abs={col_abs_max:.2f}, std={col_std:.6f}"
                )
                ...
                if col_abs_max > 1000000:
                    ...
                    logger.warning(
                        f"Applied extreme clipping [-500k, 500k] to feature '{col}' (was {col_abs_max:.2f})"
                    )
                elif col_abs_max > 200000:
                    # Large values - clip to moderate range
                    features_df[col] = np.clip(features_df[col], -150000, 150000)
                    logger.warning(
                        f"Applied large clipping [-150k, 150k] to feature '{col}' (was {col_abs_max:.2f})"
                    )
                    ...
                    logger.warning(
                        f"Applied moderate clipping [-120k, 120k] to feature '{col}' (was {col_abs_max:.2f})"
                    )
        ...
        if nan_count_after > 0:
            logger.error(
                f"Still have {nan_count_after} NaN values after cleaning, force-filling with 0"
            )
            features_df = features_df.fillna(0)

        if inf_count > 0:
            logger.error(
                f"Still have {inf_count} infinite values after cleaning, replacing with large finite values"
            )
    ...
    def _apply_intermediate_validation(self, features_df: pd.DataFrame, stage: str) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            ...
            if col_abs_max > 1000000:  # 1M+ (extreme outliers)
                ...
                logger.debug(
                    f"Intermediate clipping [-500k, 500k] on '{col}' {stage} (was {col_abs_max:.2f})"
                )
            elif col_abs_max > 200000:  # 200k+ (very high values)
                features_df[col] = np.clip(features_df[col], -150000, 150000)
                clipped_features.append((col, col_abs_max))
                logger.debug(
                    f"Intermediate clipping [-150k, 150k] on '{col}' {stage} (was {col_abs_max:.2f})"
                )
        ...
        if clipped_features:
            logger.info(
                f"Applied intermediate validation {stage}: clipped {len(clipped_features)} features"
            )
