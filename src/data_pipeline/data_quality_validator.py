...
class MarketDataValidator:
    ...
    def validate_market_data(self, df: pd.DataFrame, symbol: str) -> Tuple[pd.DataFrame, bool]:
        ...
        logger.info(
            f"✅ Validation complete for {symbol}: {'PASSED' if validation_passed else 'FAILED'}"
        )
    ...
    def _validate_ohlc_consistency(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        if high_violations.any():
            ...
            df.loc[high_violations, "high"] = df.loc[
                high_violations, ["open", "high", "low", "close"]
            ].max(axis=1)
        ...
        if low_violations.any():
            ...
            df.loc[low_violations, "low"] = df.loc[
                low_violations, ["open", "high", "low", "close"]
            ].min(axis=1)
    ...
    def _validate_time_series_integrity(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        if duplicates.any():
            ...
            severity = (
                ...
                if dup_pct > time_config["duplicate_tolerance"]
    ...
    def _generate_validation_summary(self) -> bool:
        ...
        has_critical = any(
            r.severity == ValidationSeverity.CRITICAL for r in self.validation_results
        )
