...
class FeatureSelector:
    ...
    def align_features_for_model(self, features_df: pd.DataFrame, model_type: str, 
    ...
    def _apply_model_specific_selection(self, features_df: pd.DataFrame, 
                                      model_type: str, symbol: str) -> Optional[pd.DataFrame]:
    ...
    def _apply_intelligent_feature_selection(self, features_df: pd.DataFrame, 
                                           model_type: str, symbol: str) -> pd.DataFrame:
        ...
        logger.info(f"Intelligent selection: {current_count} -> {expected_count} features for {model_type}_{symbol}")
        ...
        if current_count == expected_count:
            ...
            return self._select_best_features(features_df, current_features, expected_count, model_type)
            
    def _select_best_features(self, features_df: pd.DataFrame, feature_cols: List[str], 
                             target_count: int, model_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            selected_feature_names = [remaining_features[i] for i in selector.get_support(indices=True)]
            
            result_df = pd.DataFrame(selected_data, columns=selected_feature_names, index=feature_data.index)
            logger.info(f"Selected features for {model_type}: {selected_feature_names[:10]}..." if len(selected_feature_names) > 10 else f"Selected features: {selected_feature_names}")
    ...
    def _get_expected_feature_count(self, model_type: str, symbol: str) -> int:
        ...
        if self.feature_mappings:
            ...
            if 'feature_counts' in self.feature_mappings and model_key in self.feature_mappings['feature_counts']:
    ...
    def _get_numeric_features(self, df: pd.DataFrame) -> List[str]:
        ...
        excluded_cols = ['open', 'high', 'low', 'close', 'volume', 'quote_volume', 
                        'trades', 'taker_buy_base', 'taker_buy_quote', 'timestamp', 'target']
    ...
    def save_feature_selection(self, model_type: str, symbol: str, 
                             selected_features: List[str], method: str = "deployment") -> None:
        ...
        try:
            ...
            logger.info(f"Saved feature selection for {model_type}_{symbol}: {len(selected_features)} features")
...
class EnhancedDataPreprocessor:
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y: Optional[Union[pd.Series, np.ndarray]] = None) -> 'EnhancedDataPreprocessor':
        ...
        logger.info(f"Fitted preprocessor for {self.model_type}_{self.symbol}: {X.shape[1]} features")
        return self
