...
from .interfaces import IFeatureManager, ValidationResult, ILogger
...
class FeatureManager(BaseService, IFeatureManager):
    ...
    def detect_schema_drift(self, current_features: pd.DataFrame, reference_schema: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            metadata['high_severity_drifts'] = len([d for d in drift_results if d['severity'] == 'high'])
    ...
    def save_schema(self, symbol: str, model_type: str, features: pd.DataFrame, version: str = None) -> bool:
    ...
    def _generate_schema_from_dataframe(self, df: pd.DataFrame, version: str = "1.0.0") -> Dict[str, Any]:
    ...
    def _validate_feature_column(self, series: pd.Series, feature_name: str, feature_schema: Dict[str, Any]) -> List[str]:
        ...
        if str(series.dtype) != expected_dtype:
            errors.append(f"Feature {feature_name}: expected dtype {expected_dtype}, got {series.dtype}")
        
        # Check nullability
        if not feature_schema.get('nullable', True) and series.isnull().any():
            errors.append(f"Feature {feature_name}: contains null values but schema specifies non-nullable")
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            if min_val is not None and series.min() < min_val:
                errors.append(f"Feature {feature_name}: minimum value {series.min()} below schema minimum {min_val}")
            
            if max_val is not None and series.max() > max_val:
                errors.append(f"Feature {feature_name}: maximum value {series.max()} above schema maximum {max_val}")
    ...
    def _compare_schemas(self, reference_schema: Dict[str, Any], current_schema: Dict[str, Any]) -> List[Dict[str, Any]]:
