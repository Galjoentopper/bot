...
class ModelTransferManager:
                            ...
                            packaged_models.append(
                                {
                                    ...
                                    "size_mb": round(
                                        Path(package_path).stat().st_size / (1024 * 1024),
                                        2,
                                    ),
                                }
                            )
    ...
    def _package_model_for_transfer(
        self, model_files: Dict[str, str], model_type: str, symbol: str, output_dir: str
    ) -> Optional[str]:
...
"""
    ...
    def _validate_single_model(
        self, model_type: str, symbol: str, model_dir: Path
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            model_file_found = any(
                any(f.suffix == ext for ext in expected_extensions) for f in model_files
            )
