...
class TradingMetrics:
    ...
    def calculate_returns(
        self, prices: np.ndarray, positions: np.ndarray, include_costs: bool = True
    ) -> Dict[str, float]:
        """
    ...
    def calculate_sortino_ratio(
        self, returns: np.ndarray, target_return: float = 0.0, annualize: bool = True
    ) -> float:
        """
    ...
    def calculate_calmar_ratio(self, returns: np.ndarray, annualize: bool = True) -> float:
        """
        ...
        if annualize and total_periods > 0:
            annualized_return = (1 + total_return) ** (
                self.annualization_factor / total_periods
            ) - 1
        else:
    ...
    def calculate_all_metrics(
        ...
        metrics = {
            **return_metrics,
            ...
            "avg_trade_return": return_metrics["net_return"]
            / max(return_metrics["total_trades"], 1),
...
def find_optimal_threshold(
    ...
    logger.info(f"Optimal threshold: {best_threshold:.3f} " f"({metric}={best_metric_value:.3f})")
...
def calculate_portfolio_metrics(
    ...
    metrics["cvar_95"] = (
        float(np.mean(r_values[r_values <= var_95])) if np.any(r_values <= var_95) else 0.0
    )
    ...
    if benchmark_returns is not None:
        ...
        if len(b_values) == n:
            ...
            metrics["information_ratio"] = float(
                (metrics["excess_return"] / (te + 1e-12)) * np.sqrt(252)
            )
