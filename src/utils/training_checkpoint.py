...
class TrainingCheckpoint:
    ...
    def _ensure_checkpoint_directory(self) -> None:
        ...
        try:
            ...
            if not self.checkpoint_dir.is_dir():
                raise OSError(
                    f"Checkpoint path exists but is not a directory: {self.checkpoint_dir}"
                )
            ...
            try:
                ...
                raise OSError(
                    f"No write permission in checkpoint directory {self.checkpoint_dir}: {e}"
                )
            ...
            raise OSError(
                f"Cannot create or access checkpoint directory {self.checkpoint_dir}: {e}"
            )
    ...
    def save_checkpoint(
        ...
        try:
            ...
            if save_success:
                ...
                logger.info(
                    f"Progress: Symbol {progress.current_symbol_index + 1}/{progress.total_symbols}, "
    ...
    def record_model_failure(
        self, symbol: str, model_type: str, error: str, progress: TrainingProgress
    ) -> None:
    ...
    def should_skip_model(
        ...
        failure_count = sum(
            1
            for failure in progress.failed_models
    ...
    def load_checkpoint(
        self,
    ) -> Tuple[Optional[TrainingProgress], Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.info(
                f"Resuming: Symbol {progress.current_symbol_index + 1}/{progress.total_symbols}, "
                f"Model {progress.current_model_index + 1}/{progress.total_models}, "
                f"Fold {progress.current_fold_index + 1}/{progress.total_folds}"
            )
