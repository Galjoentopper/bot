...
class ConfigGenerator:
    ...
    def generate_trading_config(
        self, preferences: TradingPreferences, capabilities: SystemCapabilities
    ) -> Dict[str, Any]:
        ...
        config = {
            ...
            "model_loading": {
                "ensemble_weights": {"gru": 0.4, "lightgbm": 0.4, "ppo": 0.2},
    ...
    def _get_feature_config(
        self, advanced: bool, capabilities: SystemCapabilities
    ) -> Dict[str, Any]:
    ...
    def _get_lightweight_feature_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "volume_features": {"volume_sma": [10], "volume_ratio": True},
            "advanced_features": {"enabled": False},
            "statistical_features": {"enabled": False},
        }
    ...
    def interactive_config_generation(self) -> Dict[str, Any]:
        ...
        if symbol_choice == "4":
            ...
            symbols = self.symbol_presets.get(
                symbol_map.get(symbol_choice, "crypto_major"),
                self.symbol_presets["crypto_major"],
            )
        ...
        if is_training:
            advanced_features = (
                input("\n9. Enable advanced features (Fourier, Wavelet, etc.)? (Y/n): ")
                .strip()
                .lower()
                != "n"
            )
...
def main():
    ...
    try:
        ...
        print(
            f"Risk Level: {config.get('trading', {}).get('risk_management', {}).get('max_position_size', 'N/A')}"
        )
