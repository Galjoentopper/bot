...
class EnvironmentManager:
    ...
    def _determine_environment_type(
        self, memory_gb: float, cpu_count: int, gpu_available: bool
    ) -> str:
    ...
    def setup_environment(self, env_type: str, force: bool = False) -> bool:
        ...
        if env_type not in ["training", "trading"]:
            raise ValueError(
                f"Invalid environment type: {env_type}. Must be 'training' or 'trading'"
            )
    ...
    def _is_environment_compatible(self, env_type: str) -> bool:
        ...
        if env_type == "training":
            ...
            return self.current_env.total_memory_gb >= 4 and self.current_env.cpu_count >= 2
        else:
            # Trading can run on minimal resources
            return self.current_env.total_memory_gb >= 1 and self.current_env.cpu_count >= 1
    ...
    def validate_configuration(self, config_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        if self.current_env.total_memory_gb < required_memory:
            errors.append(
                f"Insufficient memory: {self.current_env.total_memory_gb:.1f}GB < {required_memory}GB required"
            )
        elif self.current_env.total_memory_gb < recommended_memory:
            warnings.append(
                f"Below recommended memory: {self.current_env.total_memory_gb:.1f}GB < {recommended_memory}GB recommended"
            )
        ...
        if not self._check_python_version(required_python):
            errors.append(
                f"Python version {self.current_env.python_version} does not meet requirement {required_python}"
            )
    ...
    def print_environment_status(self):
        ...
        print(
            f"Platform: {report['environment_info']['platform']} ({report['environment_info']['architecture']})"
        )
