...
class GRUTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.learning_rate = self.model_config.get(
            "learning_rate", 0.000001
        )  # Extremely conservative learning rate
    ...
    def build_model(self, input_size: int) -> nn.Module:
        ...
        if opt_name == "rmsprop":
            self.optimizer = optim.RMSprop(
                params, lr=self.learning_rate, alpha=0.99, eps=1e-8, weight_decay=1e-5
            )
        elif opt_name == "adamw":
            self.optimizer = optim.AdamW(params, lr=self.learning_rate, weight_decay=1e-4)
        elif opt_name == "sgd":
            self.optimizer = optim.SGD(
                params, lr=self.learning_rate, momentum=0.9, weight_decay=1e-5
            )
        else:
            # default Adam
        ...
        logger.info(
            f"Model built with {total_params:,} total parameters ({trainable_params:,} trainable)"
        )
    ...
    def prepare_data(
        ...
        logger.info(
            f"Data ranges - X_train: {X_train_max:.6f} (std: {X_train_std:.6f}), y_train: {y_train_max:.6f} (std: {y_train_std:.6f})"
        )
        ...
        if y_train_max < 0.001:
            logger.warning(
                f"Target values suspiciously small (max: {y_train_max:.8f}) - possible over-normalization or data leakage!"
            )
        if y_train_std < 0.0001:
            logger.warning(
                f"Target variance suspiciously low (std: {y_train_std:.8f}) - possible data leakage!"
            )
        ...
        logger.info(
            f"Data loaders prepared - Train: {len(train_loader)} batches, Val: {len(val_loader)} batches"
        )
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx < 3:
                logger.info(
                    f"Batch {batch_idx} - Data range: [{data.min().item():.6f}, {data.max().item():.6f}], Target range: [{target.min().item():.6f}, {target.max().item():.6f}]"
                )
            ...
            if not torch.isfinite(data).all() or not torch.isfinite(target).all():
                logger.warning(
                    f"Non-finite values found in batch {batch_idx} after sanitization, skipping"
                )
                continue
            ...
            if output_max > self._gradient_explosion_threshold:
                logger.warning(
                    f"Large model output detected ({output_max:.6f}) at batch {batch_idx}, potential gradient explosion"
                )
                self._consecutive_bad_batches += 1
                ...
                if self._consecutive_bad_batches >= 2:
                    ...
                    logger.warning(
                        f"Emergency LR reduction to {self.optimizer.param_groups[0]['lr']:.8f}"
                    )
                continue
            ...
            if loss_value < self._min_loss_threshold:
                logger.warning(
                    f"Suspiciously low loss ({loss_value:.10f}) - possible data leakage or over-fitting"
                )
            ...
            for p in self.model.parameters():
                if p.grad is not None:
                    ...
                    if torch.isnan(p.grad).any() or torch.isinf(p.grad).any():
                        logger.warning(
                            f"Non-finite gradients at batch {batch_idx}, skipping optimizer step"
                        )
            ...
            if param_count > 0:
                ...
                if total_grad_norm > 5.0:
                    logger.warning(
                        f"Large gradient norm ({total_grad_norm:.6f}) at batch {batch_idx}"
                    )
    ...
    def train(
        ...
        if MLFLOW_AVAILABLE and _mlflow_module is not None:
            mlflow_context = _mlflow_module.start_run(
                run_name=f"gru_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        else:
            from contextlib import nullcontext
        ...
        with mlflow_context:
            ...
            for epoch in range(self.epochs):
                ...
                if not torch.isfinite(torch.tensor(val_loss)):
                    logger.error(
                        f"Non-finite validation loss {val_loss} at epoch {epoch+1}, stopping training"
                    )
                    break
                    ...
                    try:
                        ...
                        if self.optimizer:
                            ...
                            _mlflow_module.log_metric(
                                "learning_rate", float(current_lr), step=epoch
                            )
                        # Log additional gradient stability metrics
                        _mlflow_module.log_metric(
                            "consecutive_bad_batches", self._consecutive_bad_batches, step=epoch
                        )
                    except Exception as e:
                ...
                if val_loss < self.best_val_loss:
                    ...
                    patience_counter = 0
                    logger.info(
                        f"Epoch {epoch+1}/{self.epochs} - New best validation loss: {val_loss:.6f}"
                    )
                else:
                    patience_counter += 1
                        ...
                        try:
                            ...
                            if MLFLOW_AVAILABLE:
                                ...
                                mlflow_pytorch.log_model(
                                    pytorch_model=self.model, artifact_path="gru_model"
                                )
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray) -> Dict[str, float]:
        ...
        logger.info(
            f"Model evaluation - RMSE: {rmse:.6f}, R²: {r2:.4f}, Dir. Acc: {directional_accuracy:.4f}"
        )
    ...
    def load_model(cls, filepath: str, config: Dict[str, Any]) -> "GRUTrainer":
        ...
        trainer.feature_names = checkpoint.get(
            "feature_names", [f"feature_{i}" for i in range(model_config["input_size"])]
        )
        ...
        if trainer.feature_count != model_config["input_size"]:
            logger.warning(
                f"Feature count mismatch: stored={trainer.feature_count}, model={model_config['input_size']}"
            )
