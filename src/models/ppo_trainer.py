...
try:
    ...
    from stable_baselines3.common.callbacks import (
        BaseCallback as SB3_BaseCallback,  # type: ignore[import-untyped]
    )
    ...
    from stable_baselines3.common.monitor import (
        Monitor as SB3_Monitor,  # type: ignore[import-untyped]
    )
    from stable_baselines3.common.vec_env import (
        DummyVecEnv as SB3_DummyVecEnv,  # type: ignore[import-untyped]
    )
                ...
                for info in self.locals["infos"]:
                    if "episode" in info:
                        ...
                        if MLFLOW_AVAILABLE and mlflow is not None:
                            mlflow.log_metric(
                                "episode_reward", info["episode"]["r"], step=current_step
                            )
                            mlflow.log_metric(
                                "episode_length", info["episode"]["l"], step=current_step
                            )
        except Exception:
...
class PPOTrainer:
    ...
    def create_environment(
        ...
        def make_train_env(*args, **kwargs):
            ...
            if total > min_len:
                ...
                env = TradingEnvironment(
                    train_data, window_start=start, window_end=end, **default_kwargs
                )
            else:
        ...
        logger.info(
            f"Created {n_envs} parallel training environments with seeds {env_seed} to {env_seed + n_envs - 1}"
        )
    ...
    def train(
        ...
        if hasattr(train_env, "observation_space"):
            ...
            self.input_size = (
                int(np.prod(self.observation_shape)) if self.observation_shape else None
            )
        ...
        with mlflow_context:
            ...
            if MLFLOW_AVAILABLE and mlflow is not None and self.model is not None and SB3_AVAILABLE:
                try:
                    ...
                    sample_observation = np.random.randn(
                        1, train_env.observation_space.shape[0]
                    ).astype(np.float32)
                    ...
                    mlflow.log_params(
                        {
                            ...
                            "action_shape": str(train_env.action_space.shape)
                            if hasattr(train_env.action_space, "shape")
                            else str(train_env.action_space.n),
                        }
                    )
                except Exception:
        ...
        if save_path:
            model_file = os.path.join(save_path, "model.zip")
            self.save_model(model_file)

        return results

    def evaluate(
        self, eval_data: pd.DataFrame, n_episodes: int = 10, deterministic: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        results = {
            ...
            "mean_total_return": float(
                np.mean([s.get("total_return", 0.0) for s in portfolio_stats])
            )
            if portfolio_stats
            else 0.0,
            "mean_sharpe_ratio": float(
                np.mean([s.get("sharpe_ratio", 0.0) for s in portfolio_stats])
            )
            if portfolio_stats
            else 0.0,
            "mean_max_drawdown": float(
                np.mean([s.get("max_drawdown", 0.0) for s in portfolio_stats])
            )
            if portfolio_stats
            else 0.0,
            "mean_win_rate": float(np.mean([s.get("win_rate", 0.0) for s in portfolio_stats]))
            if portfolio_stats
            else 0.0,
        }
    ...
    def load_model(cls, filepath: str, config: Dict[str, Any]) -> "PPOTrainer":
        ...
        try:
            ...
            if os.path.exists(metadata_path):
                ...
                if isinstance(obs_shape, (list, tuple)) and all(
                    isinstance(x, int) for x in obs_shape
                ):
