...
# Note: Individual trainers replaced by SuperiorEnsembleTrainer
...
class GRUAdapter(BaseModelAdapter):
    ...
    def predict(self, X: Union[np.ndarray, pd.DataFrame], **kwargs) -> np.ndarray:
        ...
        # Create sequences
        X_seq, _ = self.preprocessor.create_sequences(
            X_scaled, sequence_length=self.sequence_length
        )
    ...
    def get_training_history(self) -> Optional[Dict[str, List[float]]]:
        ...
        if hasattr(self.trainer, "train_losses") and hasattr(self.trainer, "val_losses"):
            return {"train_loss": self.trainer.train_losses, "val_loss": self.trainer.val_losses}
        return None
...
class PPOAdapter(BaseModelAdapter):
    ...
    def fit(
        ...
        **kwargs,
        ...
        # Train model
        results = self.trainer.train(
            train_data, eval_data, total_timesteps=total_timesteps, experiment_name=experiment_name
        )
    ...
    def predict(
        self, X: Union[np.ndarray, pd.DataFrame], deterministic: bool = True, **kwargs
    ) -> np.ndarray:
        """
    ...
    def get_artifacts(self) -> Dict[str, Any]:
        ...
        return {"model": self.trainer, "config": self.model_config, "env_kwargs": self.env_kwargs}
...
def create_model_adapter(
    model_type: str, config: Dict[str, Any], task_type: str = "regression"
) -> BaseModelAdapter:
    """
