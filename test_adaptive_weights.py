...
def test_adaptive_weights():
    ...
    for i in range(10):
        signal_generator.update_model_performance('gru', 0.8, 0.05, True)  # Good predictions, profitable
    
    # Simulate LightGBM performing poorly
    for i in range(10):
        signal_generator.update_model_performance('lightgbm', 0.3, -0.02, False)  # Poor predictions, unprofitable
    ...
    for model, stats in performance_summary.items():
        if isinstance(stats, dict) and 'accuracy' in stats:
            print(f"{model}: accuracy={stats['accuracy']:.3f}, profit_rate={stats['profit_rate']:.3f}, "
