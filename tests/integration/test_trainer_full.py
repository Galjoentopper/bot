...
def test_config_loading():
    ...
    try:
        ...
        logger.info(
            f"   - Features enabled: {sum(1 for v in config['features'].values() if isinstance(v, bool) and v)}"
        )
...
def test_dataset_builder(config):
    ...
    try:
        ...
        dataset_builder = DatasetBuilder(config=config, data_dir="./data", cache_dir="./cache_test")
        ...
        logger.info(
            f"   - Features: {[col for col in dataset.columns if not col.startswith('target_')][:10]}..."
        )
        logger.info(
            f"   - Targets: {[col for col in dataset.columns if col.startswith('target_')][:10]}..."
        )
...
def test_feature_generation(dataset_builder, config):
    ...
    try:
        ...
        dates = pd.date_range(
            start=datetime.now() - timedelta(days=100), periods=1000, freq="30min"
        )
        np.random.seed(42)
