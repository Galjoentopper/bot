...
def test_data_availability():
    ...
    try:
        ...
        print(f"✅ Data verification successful: {total_samples:,} samples across {len(data_stats)} symbols")
...
def test_dataset_preparation():
    ...
    try:
        ...
        if datasets_result.get('success') and datasets_result.get('datasets'):
            ...
            print(f"✅ Dataset preparation successful: {dataset_count} datasets, {sample_count:,} samples")
            return True
        else:
...
def test_model_trainers():
    ...
    try:
        ...
        if result.get('validation_metrics'):
            print(f"✅ Model training successful with R² = {result['validation_metrics'].get('r2', 'N/A'):.4f}")
            return True
        else:
