...
create_telegram_session() {
    ...
    tmux send-keys -t "$TELEGRAM_SESSION_NAME:telegram" "cd $SCRIPT_DIR && python3 telegram_bot_listener_systemd.py 2>&1 | tee -a $LOG_DIR/telegram_$(date +%Y%m%d_%H%M%S).log" C-m
