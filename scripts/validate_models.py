    ...
    def _find_model_paths(self) -> List[Tuple[Path, str, str]]:
        ...
        for model_type_dir in self.models_dir.iterdir():
            ...
            for symbol_dir in model_type_dir.iterdir():
                ...
                model_path = self._find_latest_model_path(symbol_dir, model_type)
                if model_path:
                    model_paths.append((model_path, model_type, symbol))
        
        return model_paths
    
    def _find_latest_model_path(self, symbol_dir: Path, model_type: str) -> Optional[Path]:
        """
        ...
        # NEW: Check nested model type directory first (e.g., models/gru/BTCEUR/gru/timestamp/)
        nested_model_dir = symbol_dir / model_type
        if nested_model_dir.exists():
            nested_timestamp_dirs = []
            for item in nested_model_dir.iterdir():
                if item.is_dir() and item.name != 'latest':
                    try:
                        # Try to parse as timestamp
                        datetime.strptime(item.name, '%Y%m%d_%H%M%S')
                        nested_timestamp_dirs.append(item)
                    except ValueError:
                        continue
            
            if nested_timestamp_dirs:
                # Return the most recent one from nested directory
                return max(nested_timestamp_dirs, key=lambda x: x.name)
        
        # Fallback: Find the most recent timestamped directory in symbol_dir
        timestamped_dirs = []
        ...
        try:
            ...
            for file_name in required_files:
                ...
                if not file_path.exists():
                    # Special case for GRU models: check for both .pt and .pth extensions
                    if model_type == 'gru' and file_name == 'model.pt':
                        alt_file_path = model_path / 'model.pth'
