...
echo "🧪 Enhanced Trader Test Script (Shell)"
...
# Start the enhanced trader in background
python "$TRADER_PATH" &
TRADER_PID=$!

echo "✅ Enhanced trader process started with PID: $TRADER_PID"

# Function to kill enhanced trader process
kill_trader() {
    echo "⏰ Timeout reached. Killing enhanced trader process..."
    if kill -TERM $TRADER_PID 2>/dev/null; then
        echo "📤 Sent SIGTERM to enhanced trader process"
        # Wait up to 10 seconds for graceful shutdown
        for i in {1..10}; do
            if ! kill -0 $TRADER_PID 2>/dev/null; then
                echo "✅ Enhanced trader process terminated gracefully"
                return 0
            fi
            sleep 1
        done
        # Force kill if still running
        echo "⚠️  Enhanced trader process didn't respond to SIGTERM, force killing..."
        kill -KILL $TRADER_PID 2>/dev/null
        echo "💀 Enhanced trader process force killed"
    else
        echo "⚠️  Enhanced trader process already finished"
    fi
}
...
if kill -0 $TRADER_PID 2>/dev/null; then
    ...
    echo "✅ Enhanced trader completed before timeout"
fi
