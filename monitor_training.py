...
def check_training_progress():
    ...
    try:
        result = subprocess.run(['pgrep', '-f', 'train_real_superior_ppo'], capture_output=True, text=True)
        ...
        try:
            with open('training_output.log', 'r') as f:
                ...
                for line in recent_lines:
                    if any(keyword in line for keyword in ['INFO', 'fps', 'iterations', 'total_timesteps', 'ep_rew_mean']):
                        print(line.strip())
    ...
    if os.path.exists('models/superior/BTCEUR'):
        print("\n📁 MODEL DIRECTORIES:")
        try:
            result = subprocess.run(['find', 'models/superior/BTCEUR', '-type', 'f'], capture_output=True, text=True)
            if result.stdout.strip():
