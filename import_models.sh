...
# Determine strict import default based on environment if not explicitly set
strict_import_default() {
    if [ -n "${STRICT_IMPORT:-}" ]; then
        return
    fi
...
configure_for_linux() {
    ...
    if [ -f "training_config.yaml" ]; then
        log_info "Creating Linux deployment configuration (notifications disabled)..."

        # Create simplified trading config for Linux (notifications disabled by default)
...
notifications:
  telegram:
    enabled: false
    # Configure bot_token and chat_id via environment variables in production
EOF
...
main() {
    ...
    # Post-import PPO health check (quick load + predict)
