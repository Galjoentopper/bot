...
scp -r server/ trader@YOUR_SERVER_IP:/opt/trading_bot/bot/
```
...
sudo cp server/systemd/trading-bot.service /etc/systemd/system/
...
sudo cp server/cron/trading_bot_monitor /etc/cron.d/
...
cp server/src/notifier/enhanced_telegram.py src/notifier/
```
...
- Deployment logs: `/opt/trading_bot/bot/logs/deployment.log`
